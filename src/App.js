...
// 1. 按日期將交易分組並加總當日損益
const dailyProfits = filteredTrades.reduce((acc, t) => {
  const date = t.date.split(' ')[0]; // 取得 YYYY-MM-DD
...
// 2. 將日期排序
const sortedDates = Object.keys(dailyProfits).sort((a, b) => new Date(a) - new Date(b));
...
const chartData = sortedDates.map((date) => {
  ...
  return {
    date: date,
    // 使用 Math.round 解決 0.000000000002 浮點數問題
  ...
  const recentTrades = filteredTrades.filter(t => new Date(t.date) >= thirtyDaysAgo);
  const recentWins = recentTrades.filter(t => t.profit > 0);
  const recentLosses = recentTrades.filter(t => t.profit < 0);

  const rAvgWin = recentWins.length ? recentWins.reduce((s, t) => s + t.profit, 0) / recentWins.length : 0;
  const rAvgLoss = recentLosses.length ? Math.abs(recentLosses.reduce((s, t) => s + t.profit, 0) / recentLosses.length) : 0;
  const recentWinRate = recentTrades.length 
    ? ((recentWins.length / recentTrades.length) * 100).toFixed(1) 
    : 0;
  // --- 2. 準備回傳結果 ---
  return {
    totalProfit,
    ...
    expectancy,
    recentRR: rAvgLoss > 0 ? (rAvgWin / rAvgLoss).toFixed(2) : "N/A",
    totalRR: (Math.abs(avgLoss) > 0 ? avgWin / Math.abs(avgLoss) : 0).toFixed(2),
    recentWinRate,
    totalWinRate: ((wins.length / filteredTrades.length) * 100).toFixed(1)
  };
}, [filteredTrades]);
    ...
    <div className="min-h-screen bg-[#0f1115] text-slate-200">
      <header className="border-b border-white/5 bg-[#161920]/80 backdrop-blur-xl sticky top-0 z-50 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <div className="p-2 bg-gradient-to-br from-blue-500 to-blue-700 rounded-lg shadow-[0_0_15px_rgba(37,99,235,0.3)]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-baseline sm:gap-2">
            <h1 className="text-xl tracking-tight text-white">
              ...
              <span className="font-extralight opacity-70 ml-1">Quant</span>
            </h1>
         ...
         <div className="flex items-center gap-2 animate-pulse ml-2">
          ...
          {/* 文字呼吸閃爍 */}
          <span className="text-[10px] text-emerald-400 font-mono font-medium tracking-tighter whitespace-nowrap">
            FIREBASE_CONNECTED
          </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          {/* 使用者名稱與頭像 */}
          <div className="flex items-center gap-3 pr-4 border-r border-white/10 hidden md:flex">
          ...
          <button
            ...
            className="p-2 rounded-lg bg-rose-500/10 text-rose-400 hover:bg-rose-500 hover:text-white transition-all duration-200"
            title="登出"
          >
            <LogOut size={18} />
          </button>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPIBox
            ...
            subValue={stats.recentWinRate} // 👈 傳入近期勝率
        ...
        <PieChart>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#161920] rounded-3xl p-6 border border-white/5 space-y-4">
              ...
              <DetailRow
                label="近一月賺賠比"
                value={stats.recentRR}
                // ✨ 邏輯：如果近一月表現比全期好，就顯示象徵獲利的綠色
                color={Number(stats.recentRR) >= Number(stats.totalRR) ? "text-emerald-400" : "text-blue-400"}
              />
            </div>
          </div>
        </div>
...
function KPIBox({ title, value, trend, subValue }) {
  const color = trend > 0 ? "text-emerald-400" : trend < 0 ? "text-rose-400" : "text-white";
  
  return (
    <div className="bg-[#161920] border border-white/5 rounded-3xl p-5 relative overflow-hidden">
      <div className="text-slate-400 text-sm font-bold uppercase mb-2">
        {title}
      </div>
      
      <div className="flex items-baseline gap-2">
        {/* ✨ 新增：近期勝率小標籤 (僅在有 subValue 時顯示) */}
        {subValue && (
          <div className="flex flex-col">
            <span className="text-[10px] text-slate-500 font-bold leading-none mb-1">近一月</span>
            <span className={`text-sm font-mono font-bold leading-none ${Number(subValue) >= Number(value.replace('%','')) ? 'text-emerald-500' : 'text-rose-500'}`}>
              {subValue}%
            </span>
          </div>
        )}
